...
for manifest in "$PLUGIN/.codex-plugin/plugin.json" "$PLUGIN/.claude-plugin/plugin.json"; do
  jq -e '.name=="agent-roles" and .version=="0.1.0"' "$manifest" >/dev/null || failed=1
done
jq -e '.name=="agent-roles" and (.plugins|length==1) and .plugins[0].name=="agent-roles" and .plugins[0].version=="0.1.0"' \
